# `monty_skill_parts` — the pydantic-monty plan variant

This directory is the **monty** variant of the LLAssembly skill. The plan language is exactly the
Python one — top-level `setup_sub_agent(...)` registrations plus a single `def main()` — but
`scripts/emulator.py` hands the script to pydantic_monty, a Python interpreter implemented in
Rust, instead of running it with `exec()`.

## What this directory contributes

`python copy_skill_to.py monty <target_dir>` copies `common_skill_parts/` (the driver, SKILL.md and
prompt assets) and overlays a single file:

| File | Role |
|---|---|
| `scripts/emulator.py` | Executes the plan; the only variant-specific code. |

**There is no `references/workers/llassembly-control-flow.md` here.** monty speaks the same plan
language as the Python variant, so the build copies
`python_skill_parts/references/workers/llassembly-control-flow.md` into the skill instead.

## How a plan is built

1. **Stage 2 — plan.** The driver tells the orchestrator to launch the `llassembly-control-flow`
   worker, which writes `plan_llassembly.py` following the borrowed control-flow spec.
2. **Stage 3 — sub-agents.** `get_sub_agents()` runs the script again through a second
   `ScriptRunnner` whose queue is `None`. Each declared sub-agent with no definition file yet gets
   one generated at `references/agents/<name>.md`.
